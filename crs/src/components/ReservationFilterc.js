...
function ReservationFilterc(props) {
  ...
  const search = () => {
    ...
    if (rsrvID !== "" && !rsrvID.match(/^\d+$/)) {
      alert("Reservation ID must contain only digits");
      return;
    }
    if (reserveTime && pickupTime) {
      if (reserveTime < pickupTime) {
        alert("Reservation time must be before pickup time");
      }
    }
    ...
    if (reserveTime && returnTime) {
      if (reserveTime < returnTime) {
        alert("Reservation time must be before return time");
      }
    }

    fetch(
      `/api/reservations/getReservations?rsrvID=${rsrvID}&userID=${props.userID}&plateID=${plate}&reserveTime=${reserveTime}&pickupTime=${pickupTime}&returnTime=${returnTime}`,
      {
        method: "GET",
      }
    )
...
export default ReservationFilterc;
